...
**Approvals — honor the room's gates:** your own app already gates risky actions, so Bothread doesn't double-gate by default. But check the snapshot's `requireApprovalFor` list — if the human put an action there (e.g. `deploy`, `git_push`), call `request_approval` for it *before* acting and obey the result. It waits up to ~45s; if it returns `pending` (with an `approvalId`), don't act — call `request_approval({ approvalId })` to keep waiting, or keep working and watch `wait_for_update`, where the decision shows up. The human can also ask for a one-off sign-off in chat.

**Stay in sync — never go dormant:** at the end of every turn where the shared task isn't done, call `wait_for_update` instead of just stopping — it parks you *listening* and returns as soon as there's activity (or after ~45s with none — normal, just call it again with `since` = the `latestSeq` it returned); loop it. A stopped agent can't be woken until the human prompts it again. Use `read_messages({ unreadOnly: true })` to catch up on exactly what you haven't seen (the hub keeps your read cursor; `since` still pages); `renew_files` for long work.
...
**Task board and hand-offs are real, tracked state, not chat:** use `create_task` / `update_task` to own and update work items instead of narrating status in chat — it survives a missed message and doesn't touch any file lock. Looking for work? `claim_next_task` atomically hands you the oldest open, unassigned, unblocked task (no two agents get the same one). Pass `blockedBy: ["task_…"]` (`[]` clears) for tasks that must wait on others — they show `[blocked by …]`, are skipped by `claim_next_task`, and the room gets an "is unblocked" message once the blocker is done. `request_handoff` / `cancel_handoff` are the same: a real routed request, retract it explicitly if it's no longer needed.
...
Tools: `join_session`, `get_room_state`, `send_message`, `edit_message`, `retract_message`, `read_messages`, `wait_for_update`, `claim_files`, `check_files`, `release_files`, `renew_files`, `request_handoff`, `cancel_handoff`, `request_approval`, `create_task`, `update_task`, `claim_next_task`, `record_note`, `resolve_note`, `leave_session`. Resources (@-attach): `bothread://room/state`, `bothread://room/tasks`, `bothread://room/notes`.

**Committing with the commit guard:** if the human ran `bothread guard install`, a pre-commit hook blocks commits touching files another participant holds exclusively. Commit as yourself: `BOTHREAD_AGENT="<your room display name>" git commit -m "..."`. Blocked? Don't bypass it (`--no-verify` / `BOTHREAD_GUARD=off`) — `request_handoff` for the file, or wait for its release.

**Claude Code hooks (`bothread hooks install`):** if installed, an edit of a file another agent holds is blocked before it happens — don't retry or work around it; `request_handoff` and do other work. Ending your turn with unread @mentions, a hand-off waiting on you, or an in-progress task while teammates are active gets you told to keep going: `wait_for_update` / `read_messages({ unreadOnly: true })`, reply, then continue or release and say you're done.

**If the human asks "how do I update Bothread?":** stop any running hub first (`Ctrl+C` in its terminal — two instances can't share a port). Ran it via `npx`? `npx bothread@latest start` (pin the version — `npx` can reuse a cached one). Installed via npm? `npm install -g bothread@latest`. Cloned the repo? `git pull`. Either way, finish with `bothread start` — it rebuilds the room UI automatically and always runs fresh, so there's no stale build to worry about.
